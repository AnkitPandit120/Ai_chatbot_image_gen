...
export async function generateImage(prompt: string, apiKey: string) {
  let response;
  try {
    response = await fetch('https://router.huggingface.co/nscale/v1/images/generations', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
    ...
    // A "Failed to fetch" here is almost always a CORS error caused by an invalid API key
    // because Hugging Face rejects invalid tokens before attaching CORS headers.
  ...
  // Check if the response is JSON (Hugging Face returns JSON for errors, or base64 image data)
  ...
  if (contentType && contentType.includes('application/json')) {
    ...
    // If it's a successful JSON response with b64_json
